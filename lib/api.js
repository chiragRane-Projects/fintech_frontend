...
const api = {
  ...
  getIntelligenceSummary: (userId, month, year) => 
    api.request(`/intelligence/summary?user_id=${userId}&month=${month}&year=${year}`),

  getPrediction: (userId, month, year) => 
    api.request(`/prediction/next-month?user_id=${userId}&month=${month}&year=${year}`),

  aiExplainer: (payload) =>
    api.request("/ai_explainer/financial-summary", {
      method: "POST",
      body: payload,
    }),
}
